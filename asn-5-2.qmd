...
Answer: Type of resource - cobalt. re_co
...
Answer: When the mine is open active97_22
...
Answer: For each year cobalt mines are open, they are predicted to have 0.23 more conflicts on average.

**Based on the regression, is the treatment effect statistically different from 0? (1 point)**

Answer: It's got the asterisks, so it's statistically significant.
...
```{r}
summary(c1<- lm(n ~ active + re_Co + active:re_Co + year_do + year_de +year, data=df))
...
**The treatment effect is still re_Co, yes it is statistically different from 0.**
...
```{r}
modelName <- felm(n ~active + re_Co + active:re_Co + year_do + year_de +year | parties, data = df)
summary(modelName)
```
...
The treatment effect is re_Co. It is statistically different from 0.
...
```{r}
modelName1 <- felm(n ~active + re_Co + active:re_Co + year_do + year_de +year + re_Ta + re_Sn | parties, data = df)
summary(modelName1)
```

Answer: The mines with tantalum and tin.
...
```{r}
modelName1 <- felm(n ~active + re_Co + active:re_Co + year_do + year_de +year + re_Ta + re_Sn + re_Co:post_active  | type + parties, data = df)
summary(modelName1)
```
...
Answer: The treatment effect is re_Co. It is statistically different from 0.
...
Answer: Question 8's regression. We include the different mines and resources, and the parties involved. We account for the variables that affect the conflicts.
...
Answer: Factors like precipitation, population, night lights can change the treated outcome.
...
Answer: Additional variables like the economic status of the DRC, price of cobalt, foreign investments in the cobalt mines.
